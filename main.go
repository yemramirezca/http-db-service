...
func addOrderHandlers(router *mux.Router, cfg config.Service) {
	...
	// orders
	router.HandleFunc("/p2/orders", orderHandler.InsertOrder).Methods(http.MethodPost)

	router.HandleFunc("/p2/orders", orderHandler.GetOrders).Methods(http.MethodGet)
	router.HandleFunc("/p2/namespace/{namespace}/orders", r.GetNamespaceOrders).Methods(http.MethodGet)

	router.HandleFunc("/p2/orders", orderHandler.DeleteOrders).Methods(http.MethodDelete)
	router.HandleFunc("/p2/namespace/{namespace}/orders", orderHandler.DeleteNamespaceOrders).Methods(http.MethodDelete)
}
...
func addAPIHandler(router *mux.Router) {
	// API
	router.HandleFunc("/p2", handler.SwaggerAPIRedirectHandler).Methods(http.MethodGet)
	router.HandleFunc("/p2/api.yaml", handler.SwaggerAPIHandler).Methods(http.MethodGet)
}
